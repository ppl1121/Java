...
public class KdTree {
	...
	public Point2D nearest(Point2D p){
		if(p==null) throw new java.lang.IllegalArgumentException();
		Nearest result = new Nearest(); 
		result.nearest_point = null;
		result.nearest_distance = Double.POSITIVE_INFINITY;
		nearest(root,p,result,0);
		return result.nearest_point;
	}
	
	private void nearest(Node x, Point2D p, Nearest result, int level){
		if(x!=null){
//			if(nearest_distance<x.rect.distanceSquaredTo(p)) return;
			...
			if(distance<result.nearest_distance){
				...
				result.nearest_point = x.p;
			}
			if(level%2==0){
				if(p.x()<x.p.x()){
					nearest(x.lb,p,result,level+1);
					if(x.rt!=null && result.nearest_distance>x.rt.rect.distanceSquaredTo(p)){
						nearest(x.rt,p,result,level+1);
					}
				}
				else{
					nearest(x.rt,p,result,level+1);
					if(x.lb!=null && result.nearest_distance>x.lb.rect.distanceSquaredTo(p)){
						nearest(x.lb,p,result,level+1);
					}
				}
			}
